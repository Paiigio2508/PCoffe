...
public interface NguoiDungRepository extends JpaRepository<NguoiDung, String> {
    @Query(value = """
        ...
        """, nativeQuery = true)
    List<NguoiDungAdminRepository> getNguoiDungByChucVu(@Param("chucVu") String chucVu);
    ...
    @Query(value = """
            select
                ...
                nd.ten as ten,
                nd.so_dien_thoai as soDienThoai,
                coalesce(nd.email, N'Không có') as email,
                coalesce(nd.ngay_sinh, N'Không có') as ngaySinh,
                nd.gioi_tinh as gioiTinh,
                nd.cccd as cccd,
                coalesce(nd.anh, N'Không có') as anh,
                nd.trang_thai as trangThai
            from nguoi_dung nd
            where
                (:#{#search.tenTimKiem} IS NULL 
                 OR nd.ma like %:#{#search.tenTimKiem}% 
                 OR nd.ten like %:#{#search.tenTimKiem}% 
                 OR nd.so_dien_thoai like %:#{#search.tenTimKiem}%)
                AND (:#{#search.trangThai} IS NULL 
                 OR nd.trang_thai = :#{#search.trangThai})
                AND nd.chuc_vu = :chucVu
            order by ma desc
            """, nativeQuery = true)
    List<NguoiDungAdminRepository> searchNguoiDung(SearchTenAndTrangThaiRequest search, @Param("chucVu") String chucVu);
